...
#                $SATIP_BIN -s $SATIP_SERVER -p $SATIP_PORT -d /dev/vtunerc$i -f $frontend &
...
install_vtuner_and_satip() {
    ...
    if modinfo vtunerc &>/dev/null; then
        ...
        sudo apt update
        sudo apt install -y dkms git build-essential linux-headers-$(uname -r) libcap-dev psmisc w_scan
...
wait'
ExecStop=/usr/bin/pkill -f $SATIP_BIN
...
EOF
    ...
    sudo systemctl enable --now vtuner-satip.service
...
show_loaded_menu() {
    while true; do
        if is_driver_loaded; then
            ...
            echo "+---------------------------------------------+"
            echo "| 1) vtuner Module entladen                   |"
            echo "| 2) Neu laden (Entladen + neue devices)      |"
            echo "| 3) SAT>IP-Verbindungen beenden              |"
            echo "| 4) Alle vtuner/SAT>IP anzeigen              |"
            echo "| 5) VDR neu starten                          |"
            echo "| 6) systemd-Service vtuner-satip erstellen   |"
            echo "| 7) systemd-Service vtuner-satip löschen     |"
            echo "| 8) vtuner/satip installieren oder prüfen    |"
            echo "| 9) Abbrechen                                |"
            echo "+---------------------------------------------+"
            read -p "=> " choice
            echo ""
