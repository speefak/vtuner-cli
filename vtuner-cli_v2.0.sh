...
show_status () {
    ...
    SatIPProcesses=$(pgrep -fa "$SatipBIN" | grep -v "/bin/bash -c  $SatipBIN"|grep -v "/bin/bash")
    vTunerUsed=
    vTunerUnUsed=
...
ExecStop=/bin/bash -c 'pkill -f /usr/local/bin/satip || true'
...
EOF
    ...
    echo ""
    echo " Systemdienst (vtuner-satip.service) steuern:"
    echo "   sudo systemctl <start|stop|status> vtuner-satip.service -l"
}
#------------------------------------------------------------------------------------------------------------------------------------------------
